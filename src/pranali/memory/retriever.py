...
class MemoryRetriever:
    ...
    async def retrieve(self, user_id: uuid.UUID, top_k: int = 8, embedding: Optional[List[float]] = None) -> List[RetrievedMemory]:
        ...
        # 1. Retrieve by vector similarity if embedding provided
        if embedding:
            vector_results = await self.repository.search_similar(user_id, embedding, limit=top_k)
            memories.extend(vector_results)

        # 2. Retrieve recent active memories
        recent_results = await self.repository.get_recent_active_memories(user_id, limit=top_k)
        memories.extend(recent_results)

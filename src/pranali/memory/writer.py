...
class MemoryWriter:
    ...
    async def write(self, candidate: MemoryCreate, embedding: Optional[list[float]] = None, embedding_model: Optional[str] = None) -> MemoryItem:
        ...
        # 1. Check for existing duplicate by exact match on title + type
        existing = await self.repository.get_by_exact_match(candidate.user_id, candidate.memory_type, candidate.title)

        # 2. Check for duplicate by vector similarity if we have an embedding
        ...
        if existing:
            ...
            existing.confidence_score = min(1.0, existing.confidence_score + 0.1) # Boost confidence
            if embedding:

...
class MemoryService:
    ...
    async def extract_and_store_from_turn(self, user_id: uuid.UUID, user_name: str, recent_turns: str, source_event_id: Optional[uuid.UUID] = None) -> None:
        ...
        for candidate in candidates:
            # Phase 2 decision engine filters out secrets and low value events
            decision = self.decision_engine.decide(candidate, similar_memories=[], active_goals=[])

            if decision == MemoryDecision.IGNORE or decision == MemoryDecision.EVENT_ONLY:
                continue
